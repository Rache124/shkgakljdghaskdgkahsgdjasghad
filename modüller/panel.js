...
module.exports = (client) => {
  ...
  const bilgiler = {
    oauthSecret: "-PrOZzM3pV9wknqbGbhuNVWqddABbcNy",
    callbackURL: `https://aware-hurricane-syzygy.glitch.me/callback`,
    domain: `https://aware-hurricane-syzygy.glitch.me/`
  };
...
//Tovbe tovbe kufurlere bak 
 ...
 const renderTemplate = (res, req, template, data = {}) => {
  ...
  app.get("/callback", passport.authenticate("discord", { failureRedirect: "/autherror" }), async (req, res) => {
    ...
    client.channels.cache.get("820037892859494400").send(`**${client.users.cache.get(req.user.id).tag}** adlı kullanıcı Web Paneline Discord hesabıyla giriş yaptı!`)
...
res.redirect("/panel/"+req.params.guildID+"/genel");
  ...
  app.post("/panel/:sunucuID/yonet", girisGerekli, (req, res) => {
    ...
    if (req.body['ban']) {
      if (sunucu.members.get(client.user.id).permissions.has("BAN_MEMBERS") === false) return res.json({"hata":"Botun "+sunucu.name+" adlı sunucuda Üyeleri Yasakla (BAN_MEMBERS) izni olmadığı için "+client.users.cache.get(req.body['ban']).tag+" adlı üye yasaklanamıyor."});
    }
    if (req.body['unban']) {
      require('request')({
    ...
    if (req.body['kick']) {
      if (sunucu.members.get(client.user.id).permissions.has("KICK_MEMBERS") === false) return res.json({"hata":"Botun "+sunucu.name+" adlı sunucuda Üyeleri At (KICK_MEMBERS) izni olmadığı için "+client.users.cache.get(req.body['kick']).tag+" adlı üye atılamıyor."}); 
    }
  ...
  app.get("/ekle", (req, res) => {
   ...
   app.get("/seviye", (req, res) => {
    res.redirect(`https://aware-hurricane-syzygy.glitch.me/kullanici/${req.params.kullaniciID}/yonet`);
  });
